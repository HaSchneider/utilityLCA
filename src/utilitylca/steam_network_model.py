...
from . import overwrite_tespy as ot
import copy
...
def create_steam_net(steam_lca):
    ...
    steam_losses = ot.Sink('steam losses')
    steam_leak= Splitter("steam leak")
    makeup_leak =ot.Source('leak makeup')
    makeup_trap =ot.Source('trap makeup')
    makeup=ot.Source("Make-up water")
    blowdown= ot.Sink("blowdown wastewater")
    cond_waste= ot.Sink("pipe condensate wastewater")
    ...
    dummy_sink2= ot.Sink('dummy sink2')
    injection_source =ot.Source('injection_source')
    ...
    pipe_diss_sink =ot.PowerSink('pipe dissipative losses sink')
    ...
    heat_sink =ot.PowerSink('heat sink')
    e_heat_sink =PowerConnection( hex_heat_sink,'heat',heat_sink, 'power', label='e_heat_sink')

    pump_psource = ot.PowerSource('feedpump powersource')
